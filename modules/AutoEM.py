...
def run_pulchra(dir,pulchar_path,pdbfile,emid,pdbid):
    cspath = os.path.join(dir,f'chain_split/{emid}_{pdbid}')
...
class Solver:
    def __init__(self,dynamic_config):
        ...
        if dynamic_config.protocol =='temp_free':
            self.method_name='EModelX'
        elif dynamic_config.protocol =='temp_flex':
            self.method_name='EModelX(+AF)'
        elif dynamic_config.protocol =='seq_free':
            self.method_name='EModelX(noseq)'
        try:
            self.emid = dynamic_config.EM_map.split('/')[-1].split('emd_')[-1].split('.')[0]
        except:
            self.emid='unknownEM'
        try:
            self.pdbid = dynamic_config.fasta.split('/')[-1].split('.fasta')[0]
        except:
        ...
        self.ca_model_pdb = os.path.join(self.dynamic_config.output_dir,f'{self.pdbid}_{self.method_name}_ca_model.pdb')
        self.time_log = os.path.join(self.dynamic_config.output_dir,f'time_cost_{self.pdbid}_{self.method_name}.csv')
    ...
    def run(self,BB_model,CA_model,AA_model):
        ...
        if self.dynamic_config.protocol in ['temp_free','temp_flex']:
            ...
            print('gapThreading finished in {} seconds'.format(round(self.time_cost['gapThreading'])))

        if self.dynamic_config.run_pulchra:
            print('running pulchra...')
            start_time=time.time()
            all_atom_model=run_pulchra(self.dynamic_config.output_dir,self.dynamic_config.pulchra_path,self.ca_model_pdb,self.emid,self.pdbid)
            self.time_cost['run_pulchra']=time.time()-start_time
            print('run_pulchra finished in {} seconds'.format(round(self.time_cost['run_pulchra'])))
        
            if all_atom_model and self.dynamic_config.run_phenix:
    ...
    def fragModeling(self):
        ...
        if self.dynamic_config.protocol =='seq_free':
            with open(os.path.join(self.dynamic_config.output_dir,f'{self.pdbid}_{self.method_name}_ca_model.pdb'),'w') as w:
                atom_ix = 0
    ...
    def seqMapAligning(self):
        ...
        if self.dynamic_config.protocol == 'temp_free':
            ...
            self.time_cost['seqStructAlignWithTemplate']=time.time()-start_time
            print('seqStructAlignWithTemplate finished in {} seconds'.format(round(self.time_cost['seqStructAlignWithTemplate'])))
    ...
    def initialModeling(self):
        ...
        with open(os.path.join(self.dynamic_config.output_dir,f'{self.pdbid}_{self.method_name}(init)_ca_model.pdb'),'w') as w:
            j = 0
            atom_ix=0
            for fasta_ix, fasta_name in enumerate(self.fastas):
                this_fasta = self.fastas[fasta_name]
    ...
    def phenix_refine(self,all_atom_model):
        ...
        EM_map=os.path.abspath(self.dynamic_config.EM_map)
        all_atom_model=os.path.abspath(all_atom_model)
        cmd=f'bash phenix.sh {phenix_act} {output_dir} "phenix.real_space_refine {all_atom_model} {EM_map} {phenix_param} resolution={self.resol}"'
